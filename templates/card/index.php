...
<?php while ( have_posts() ) : the_post();
  ...
  <article class="ph-card" id="post-<?php the_ID(); ?>">
    <?php if ( has_post_thumbnail() ) : ?>
      <a href="<?php echo esc_url( get_permalink( $id ) ); ?>" class="ph-card-thumb">
        <?php echo get_the_post_thumbnail( $id, 'ph-single', array( 'class' => 'ph-card-image', 'alt' => esc_attr( get_the_title( $id ) ) ) ); ?>
      </a>
    <?php endif; ?>

    <div>
      <h3 class="ph-title"><a href="<?php echo esc_url( get_permalink( $id ) ); ?>"><?php echo esc_html( get_the_title( $id ) ); ?></a></h3>
    ...
    <div class="ph-meta" style="margin-top:auto;display:flex;align-items:center;justify-content:space-between;">
      ...
      <div style="display:flex;gap:8px;align-items:center;">
        ...
        <a class="ph-btn" href="<?php echo esc_url( get_permalink( $id ) ); ?>"><?php esc_html_e( 'Open', 'pluginhub-pro' ); ?></a>
      </div>
    </div>
  </article>
<?php endwhile; ?>
